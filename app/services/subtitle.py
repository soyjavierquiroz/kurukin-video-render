...
SEMANTIC_BOUNDARY_PREPOSITIONS = {"a"}
...
def _semantic_boundary_strength(left_word, right_word=""):
    ...
    if right_token in SEMANTIC_BOUNDARY_CONNECTORS:
        return 45
    if left_token in SEMANTIC_BOUNDARY_PREPOSITIONS:
        return 45
...
def _has_readable_semantic_split(words, start, end):
    """Whether a full cue has a compact, natural internal split."""
    for split in range(start + 3, end - 2):
        left = words[start:split]
        right = words[split:end]
        if (
            len(_join_semantic_words(left)) < SEMANTIC_SUBTITLE_TARGET_MIN_CHARS
            or len(_join_semantic_words(right)) < SEMANTIC_SUBTITLE_TARGET_MIN_CHARS
        ):
            continue
        if _semantic_boundary_strength(left[-1]["text"], right[0]["text"]) >= 35:
            return True
    return False
...
def _balanced_span_score(words, start, end, total_words):
    ...
    if (
        ...
        and not _has_sentence_end(selected[-1]["text"])
        and word_count > SEMANTIC_SUBTITLE_TARGET_MIN_WORDS + 1
    ):
        # The upper word cap remains valid, but a continuing cue beyond the
        # compact visual target should yield to a balanced nearby split.
        score -= 60 * (word_count - (SEMANTIC_SUBTITLE_TARGET_MIN_WORDS + 1))
    ...
    if end == total_words:
        ...
        if (
            word_count >= SEMANTIC_SUBTITLE_TARGET_MAX_WORDS
            and char_count > target_chars
            and _has_readable_semantic_split(words, start, end)
        ):
            score -= 120
...
def _preserves_compact_visual_split(words, start, end, next_end):
    left = words[start:end]
    right = words[end:next_end]
    if str(left[0]["text"]).startswith(("¿", "¡")):
        return True
    return (
        _normalize_token_for_subtitle_alignment(left[-1]["text"])
        in SEMANTIC_BOUNDARY_PREPOSITIONS
        and len(left) >= 3
        and len(right) >= 3
        and len(left) + len(right) <= SEMANTIC_SUBTITLE_TARGET_MAX_WORDS
        and len(_join_semantic_words(left)) >= SEMANTIC_SUBTITLE_TARGET_MIN_CHARS
        and len(_join_semantic_words(right)) >= SEMANTIC_SUBTITLE_TARGET_MIN_CHARS
    )


def _sentence_word_count(words, offset):
    start = offset
    while start > 0 and not _has_sentence_end(words[start - 1]["text"]):
        start -= 1
    end = offset
    while end < len(words) and not _has_sentence_end(words[end]["text"]):
        end += 1
    return end - start + (end < len(words))
...
def _rebalance_semantic_spans(words, spans):
    ...
    while index < len(repaired) - 1:
        ...
        merged_within_visual_target = (
            len(merged_text) <= SEMANTIC_SUBTITLE_TARGET_MAX_CHARS
            or (
                _has_sentence_end(words[next_end - 1]["text"])
                and len(merged_text) <= SEMANTIC_SUBTITLE_HARD_CUE_CHARS
                and _sentence_word_count(words, start)
                <= SEMANTIC_SUBTITLE_TARGET_MAX_WORDS * 2
            )
        )
        should_merge_dangling_boundary = (
            _boundary_is_dangling(words, end, next_end)
            and merged_within_visual_target
            and not _preserves_compact_visual_split(words, start, end, next_end)
        )
        if end != next_start or not should_merge_dangling_boundary:
            index += 1
            continue
